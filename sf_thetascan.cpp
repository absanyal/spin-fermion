...
int Lx, Ly, numw, Mx, My, nx, ny, thetanum;
float minw, maxw, JH, G, filling;
...
int main(int argc, char *argv[])
{
    if (argc != 6)
    {
        std::cerr << "Enter Lx, Ly, JH, thetanum, filling"
                  << endl;
        exit(1);
    }
    Lx = std::atoi(argv[1]);
    ...
    int numparticles;
    numparticles = filling * Lx * Ly;
    cout << "Number of particles: " << numparticles << endl;

    for (int theta_cx = 0; theta_cx < thetanum; theta_cx++)
    {
        for (int theta_cy = 0; theta_cy < thetanum; theta_cy++)

        {

            // Theta and phi arrays
            double d_theta_x = theta_cx * 2.0 * 3.1415 / (thetanum * 1.0);
            double d_theta_y = theta_cy * 2.0 * 3.1415 / (thetanum * 1.0);
            double d_phi_x = 0.0;
            double d_phi_y = 0.0;
            ...
            {
                totale += eigs_[i] / (numparticles * 1.0);
            }

            outfile << d_theta_x << "\t" << d_theta_y << "\t"
                    << "\t" << totale << endl;
        }
        outfile << endl;
    }
